...
app.get("/archipelago/locations", (req, res) => {
  ...
  apDb.all(sql, [], (err, rows) => {
    ...
    const locations = rows.map(row => {
      ...
      const name = seen[row.title] > 1 ? `${row.title} (${seen[row.title]})` : row.title;
...
io.on("connection", (socket) => {
  ...
  try {
    ...
    // Get one replacement item that works for ALL players
    ...
    // Replace the item at the same index for every player in the lobby
    for (const p of lobby.players) {
      if (itemIndex < p.items.length && !p.items[itemIndex].found) {
        p.items[itemIndex] = replacement;
      }
    }

    // Broadcast the replacement to everyone in the lobby
    io.to(lobbyId).emit("game:itemReplaced", {
      itemIndex,
      newItem: {
    ...
    console.log(`⏭️ Replaced item ${itemIndex} for ALL players in lobby ${lobbyId}`);
  } catch (err) {
