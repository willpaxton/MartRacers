...
###Source Code:
Source code is split into two primary branches, one for front-end development (FrontEnd) and one for back-end/prototyping (prototype).  These two branches will be merged in a future sprint as we work towards a final deliverable.  
Final Update: these branches have now been condensed into a main branch for development, and a deploy branch which is the live version hosted at martracers.quaglabs.net.  Main contains the final version of this project.

###Coding Standards: 
As most developers are doing their own work in their own parts of the project, no major coding standards have been implemented. As we begin to merge parts of the project, the team will discuss coding standards and agree on a standard for this.  
Final Update: We had to adopt standards around spirnt 6, as we realized that merging the two parts of the project was very painful since there was no coding standard.  Although it took the rest of the sprints, we were able to get the project to a point where it was uniform.

###Documenation Standards: 
Documents follow templates shared around the team.  Documentation also takes the place of code comments within the code to explain reasoning and functionality of some implementations.  We plan to work more with creating structured documentation in the next few sprints.

###Development Environment:  
The project is built on top of Node.JS.  The server is a Node.JS application which is able to serve up a webpage created in HTML/CSS.  The server utilizes the Socket.IO platform in order to facilitate real-time client-server communication for the game.  The server also utilizes a SQLite database for product and gameplay data storage.  

###Deployment Environment: 
The project is currently ran manually using a command-line interface.
